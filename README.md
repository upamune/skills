...
- [init-project](./skills/engineering/init-project/SKILL.md): Go / TypeScript Web アプリ (Bun + TanStack Start + Tailwind / shadcn/ui + Effect + Ultracite) の新規プロジェクトを mise・CI（format/lint/typecheck/test/build, pinact）・backlog・upamune/skills 込みで立ち上げる。新しいリポジトリで最初に一度だけ実行する。
